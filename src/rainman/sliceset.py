...
class SliceSet(object):
  ...
  def add(self, slice_):
    self.assert_valid_slice(slice_)
    ...
    while k < len(self._slices) - 1:
      if self._slices[k].stop < self._slices[k + 1].start:
        break
      self._slices[k] = self._merge(self._slices[k], self._slices.pop(k + 1))
  ...
  def __contains__(self, slice_):
    ...
    def check(index):
      return index >= 0 and len(self._slices) > index and (
          self._contains(slice_, self._slices[index]))
